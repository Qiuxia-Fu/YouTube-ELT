...
def mock_postgres_conn_vars():
    ...
    with mock.patch.dict("os.environ", AIRFLOW_CONN_POSETGRES_DB_YT_ELT=conn_uri):
        yield Connection.get_connection_from_secrets(conn_id="POSETGRES_DB_YT_ELT")
...
# -------------integration test using real credentials
@pytest.fixture()
...
@pytest.fixture()  # test postgres connection
def real_postgres_connection():
    ...
    try:
        ...
        pytest.fail(f"Failed to connectto the dababse:{e}")
