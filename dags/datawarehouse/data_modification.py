...
def insert_rows(cur, conn, schema, row):
    try:  # read data from json to staging area schema
        if schema == 'staging':
            video_id = 'video_id'
        ...
        else:  # read data from previous layer to core schema
            video_id = 'Video_ID'
...
def delete_rows(cur, conn, schema, ids_to_delete):
    try:  # convert variables into proper sql format string, using pythong string join method
        ...
        logger.error(f"Error deleting row with Video_ID:{ids_to_delete}")
        raise e
